/*
 * Copyright (C) TrinityCore <http://www.trinitycore.org/> 2021
 *
...
public:
    ...
    bool OnTrigger(Player* player, AreaTriggerEntry const* /*areaTrigger*/d) override
    {
    ...
    struct npc_tushui_monk_on_poleAI : public ScriptedAI
    {
        ...
        void SpellHit(WorldObject* caster, SpellInfo const* spellInfo) override
        {
            if (spellInfo->Id == SPELL_FORCECAST_RIDE_POLE)
                DoCastAOE(SPELL_MONK_RIDE_POLE, true);
        }
        ...
        void DamageTaken(Unit* attacker, uint32& damage) override
        {
            if (damage >= me->GetHealth())
            {
                ...
                me->RemoveAllAuras();
                me->SetFaction(35);
                me->SetUnitFlags(UnitFlags(UNIT_FLAG_IMMUNE_TO_PC));
                me->AttackStop();
                attacker->AttackStop();
                me->_ExitVehicle();
...
class spell_ruk_ruk_ooksplosions : public AuraScript
{
    ...
    void HandleEffectPeriodic(AuraEffect const* /*aurEff*/)
    {
        ...
        caster->CastSpell(Position{ x, y, z }, SPELL_OOKSPLOSIONS_TRIGGERED, true);
    }
...
public:
    ...
    struct npc_ruk_ruk_rocketAI : public ScriptedAI
    {
        ...
        {
            Creature* owner = owner->ToCreature();
            if (!owner)
                return;
...
public:
    ...
    struct npc_zhaorenAI : public ScriptedAI
    {
        ...
        void SpellHit(WorldObject* caster, SpellInfo const* spellInfo) override
        {
            ...
            if (spellInfo->Id == SPELL_OVERPACKED_FIREWORK)
            {
                if (!me->IsInCombat())
...
public:
    ...
    struct npc_aysa_vordraka_fightAI : public ScriptedAI
    {
        ...
        void SpellHit(WorldObject* caster, SpellInfo const* spellInfo) override
        {
            if (spellInfo->Id == SPELL_DEEP_SEA_SMASH)
            {
