...
struct boss_helix_gearbreaker : public BossAI
{
    ...
    void SpellHitTarget(WorldObject* target, SpellInfo const* spellInfo) override
    {
        ...
        if (spellInfo->Id == SPELL_RIDE_VEHICLE)
    ...
    void PassengerBoarded(Unit* passenger, int8 seatId, bool apply) override
    {
        ...
        if (Creature* helix = passenger->ToCreature())
        {
            if (apply)
            {
                ...
                {
                    helix->RemoveUnitFlag(UNIT_FLAG_NOT_SELECTABLE);
                    DoCastSelf(SPELL_HOLD_THROWN);
                }
                else if (seatId == SEAT_HELIX)
                {
                    helix->AddUnitFlag(UNIT_FLAG_NOT_SELECTABLE);
...
class spell_helix_sticky_bomb_periodic_trigger : public AuraScript
{
    ...
    void HandleTick(AuraEffect const* /*aurEff*/)
    {
        ...
        SpellInfo const* spell = sSpellMgr->GetSpellInfo(GetSpellInfo()->GetEffect(EFFECT_0).TriggerSpell, DIFFICULTY_NONE);
        if (!spell)
            return;

        if (Player* player = target->SelectNearestPlayer(spell->GetEffect(EFFECT_0).CalcRadius()))
        {
    ...
    void HandleLaunch(SpellEffIndex effIndex)
    {
        ...
        if (Unit* caster = GetCaster())
        {
            ...
            if (Creature* creature = caster->ToCreature())
            {
                ...
                creature->setAttackTimer(BASE_ATTACK, 1500);
            }
        }
    }
