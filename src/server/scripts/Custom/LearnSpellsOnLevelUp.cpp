/*
	Auto Learn Spell for Trinitycore
	Code: Unknown
	Updated and Modified: by Nekro
	July 2020
...
int config_LearnSpellsEnabled = 1;
...
class LearnSpellsOnLevelUp : public PlayerScript
{
	...
	void LearnSpellsForNewLevel(Player* player, uint8 level)
	{
		...
		for (uint32 i = 0; i < sSpellMgr->GetSpellInfoStoreSize(); ++i)
		{
			...
			{
				if (itr->second->ID == spellInfo->Id && itr->second->RaceMask == 0 && itr->second->AcquireMethod == 0)
				{
                    valid = true;
                    ...
                    if (GetTalentSpellPos(itr->second->ID))
...
class LearnSpellsOnLevelUp_Config : public WorldScript
{
	public:LearnSpellsOnLevelUp_Config() : WorldScript("LearnSpellsOnLevelUp_Config") {};
		
		virtual void OnConfigLoad(bool /*reload*/)
		{
			config_LearnSpellsEnabled = sConfigMgr->GetIntDefault("LearnSpellsOnLevelUp.Enable", 1);
			TC_LOG_INFO("server.loading", "=======================================================");
			TC_LOG_INFO("server.loading", "==             Loaded Auto Learn Spell               ==");
			TC_LOG_INFO("server.loading", "=======================================================");
...
void AddSC_LearnSpellsOnLevelUp()
{
	...
	new LearnSpellsOnLevelUp_Config();
}