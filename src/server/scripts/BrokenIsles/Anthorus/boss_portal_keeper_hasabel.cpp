...
enum Talk
{
    EVENT_ON_MOVEINLINEOFSIGHT = 0,
    EVENT_ON_ENTERCOMBAT = 1,
    SPELL_COLLAPSING_WORLD2 = 2,
    EVENT_ON_HP90 = 3,
    EVENT_ON_HP60 = 4,
    EVENT_ON_HP30 = 5,
    EVENT_ON_JUSTDIED = 6
...
struct boss_portal_keeper_hasabel : public BossAI
{
    ...
    void MoveInLineOfSight(Unit* who) override
    {
        ...
        {
            IsLock = true;
            Talk(EVENT_ON_MOVEINLINEOFSIGHT);
...
struct npc_lord_eilgar_122213 : public ScriptedAI
{
    ...
    {
        EventMap().ScheduleEvent(SPELL_UNSTABLE_PORTAL, 8s);
        EventMap().ScheduleEvent(SPELL_CORRUPT, 8s);
        EventMap().ScheduleEvent(SPELL_DELUSIONS, 15s);
    }
...
struct npc_lady_dacidion_122212 : public ScriptedAI
{
    ...
    {
        EventMap().ScheduleEvent(SPELL_UNSTABLE_PORTAL, 8s);
        EventMap().ScheduleEvent(SPELL_FELSILK_WRAP, 17s);
        EventMap().ScheduleEvent(SPELL_POISON_ESSENCE, 10s);
        EventMap().ScheduleEvent(SPELL_LEECH_ESSENCE, 10s);
    }
...
struct npc_vulcanar_122211 : public ScriptedAI
{
    ...
    {
        EventMap().ScheduleEvent(SPELL_UNSTABLE_PORTAL, 8s);
        EventMap().ScheduleEvent(SPELL_SUPERNOVA, 8s);
        EventMap().ScheduleEvent(SPELL_FLAMES_OF_XOROTH, 7s);
    }
