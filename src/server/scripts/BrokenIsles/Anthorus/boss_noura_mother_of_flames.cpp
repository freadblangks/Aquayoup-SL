...
struct npc_asara_mother_of_night_122467 : public ScriptedAI
{
    ...
    {
        Talk(0);
       EventMap().ScheduleEvent(SPELL_SHADOW_BLADES, 11s);
       EventMap().ScheduleEvent(SPELL_STORM_OF_DARKNESS, 26s);
       EventMap().ScheduleEvent(SPELL_TOUCH_OF_DARKNESS, 5s);
    }
...
struct npc_diima_mother_of_gloom_122469 : public ScriptedAI
{
    ...
    {
        Talk(0);
        EventMap().ScheduleEvent(SPELL_FLASHFREEZE, 10s);
        EventMap().ScheduleEvent(SPELL_CHILLED_BLOOD_2, 6s, 7s);
        EventMap().ScheduleEvent(SPELL_ORB_OF_FROST, 30s);
    }
...
struct npc_torment_of_norgannon_123503 : public ScriptedAI
{
    npc_torment_of_norgannon_123503(Creature* creature) : ScriptedAI(creature)
    {
        ...
        me->AddAura(SPELL_VISAGE_OF_THE_TITAN, me);
        me->GetMotionMaster()->MoveTakeoff(250.0f, 60000);
    }
...
struct npc_torment_of_amanthul_125837 : public ScriptedAI
{
    ...
    {
        EventMap().ScheduleEvent(SPELL_MACHINATIONS_OF_AMANTHUL, 5s);
    }
...
struct npc_torment_of_golganneth_124164 : public ScriptedAI
{
    ...
    {
        SetCombatMovement(false);
        EventMap().ScheduleEvent(SPELL_FLASHFREEZE, 5s);
    }
...
struct npc_shadow_blade_123086 : public ScriptedAI
{
    ...
    void DoAction(int32 param) override
    {
        ...
        {
        case 1:
            me->GetMotionMaster()->MoveTakeoff(100.0f, 20000);
            break;
        }
    }
