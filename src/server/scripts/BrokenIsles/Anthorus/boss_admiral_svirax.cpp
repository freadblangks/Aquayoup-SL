...
struct boss_admiral_svirax : public BossAI
{
    ...
    void JustDied(Unit* /*killer*/) override
    {
        ...
        if (instance)
            instance->DoPlayConversation(5525);
    }
...
struct npc_chief_engineer_ishkar_122369 : public ScriptedAI
{
    ...
    void EnterCombat(Unit* /*attacker*/)
    {
        ...
        EventMap().ScheduleEvent(SPELL_EXPLOIT_WEAKNESS, 8s);
        if (IsMythic())
            EventMap().ScheduleEvent(SPELL_SHOCK_GRENADE, 15s);
        EventMap().ScheduleEvent(SPELL_ASSUME_COMMAND, 90s);
        EventMap().ScheduleEvent(SPELL_CHAOS_PULSE, 15s);
        //In Pod
        EventMap().ScheduleEvent(SPELL_ENTROPIC_MINE, 5s);
    }
...
struct npc_general_erodus_122333 : public ScriptedAI
{
    ...
    void EnterCombat(Unit* /*attacker*/)
    {
        ...
        EventMap().ScheduleEvent(SPELL_CHAOS_PULSE, 15s);
        //In Pod
        EventMap().ScheduleEvent(SPELL_SUMMON_REINFORCEMENTS, 8s);
    }
