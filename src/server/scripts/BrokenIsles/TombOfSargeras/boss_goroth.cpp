...
enum Spells
{
    ...
    PELLVISUAL_INFERNAL_SPIKE_DESTROY
};
...
class spell_infernal_burning : public SpellScript
{
    ...
    void BeforeCastHandler()
    {
        ...
        //caster->GetCreatureListWithEntryInGrid(infernalSpikes, NPC_INFERNAL_SPIKE, 200.0f);
             ...
             //caster->GetThreatManager().GetThreat(nullptr);
             //Creature* spike;
                //spike->IsInBetween(GetCaster(), spike->SelectNearestTarget(2.0f));
                    //spike->SelectNearestTarget()->AddAura(SPELL_INFERNAL_SPIKE_PROTECTION, spike);
    }

    void AfterCastHandler()
    {
        if (Creature* caster = GetCaster()->ToCreature())
                caster->GetThreatManager().GetThreat(caster);
        //if (Player* me = me)
               // me->RemoveAurasDueToSpell(SPELL_INFERNAL_SPIKE_PROTECTION);
    }
...
struct at_goroth_shattering_star : AreaTriggerAI
{
    ...
    void OnUnitEnter(Unit* unit, Player* me)
    {
        if (unit->GetEntry() == NPC_INFERNAL_SPIKE)
        {
            ++_infernalSpikeTouched;
            me->GetGUID(), SPELLVISUAL_INFERNAL_SPIKE_DESTROY;
