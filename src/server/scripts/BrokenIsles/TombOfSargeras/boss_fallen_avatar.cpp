...
struct boss_fallen_avatar : public BossAI
{
    ...
    {
        _JustDied();
        instance->DoPlayConversation(4955);
    }
    ...
    void SwitchPhase()
    {
        if(PhaseStatus == PHASE_01)
        {
            ...
            AddTimedDelayedOperation(3500, [this]() -> void
            {
                ...
                me->NearTeleportTo(Position(6561.1f, -791.82f, 1509.91f, 6.2429f));
                DoCastSelf(SPELL_ANNIHILATION);
            });
    ...
    void ExecuteEvent(uint32 eventId) override
    {
        ...
        {
            me->SummonCreature(NPC_MAIDEN_OF_VALOR, me->GetPositionWithOffset(25.0f), TEMPSUMMON_MANUAL_DESPAWN, WEEK);
            break;
        }
        ...
        default:
            if  (me->GetDistance2d(me->SelectNearestPlayer(-1))>me->GetCombatReach())
        ...
        if (who->IsPlayer() && me->IsWithinDist(who, 25.0f, false) && !IsLock)
        {
            ...
            if (Creature* jidan = me->FindNearestCreature(120867, 50.0f, true))
            {
                AddTimedDelayedOperation(25000, [this, jidan]() -> void
                {
                    ...
                    me->NearTeleportTo(me->GetPositionWithOffset(20.0f));
                });
...
struct npc_maiden_of_valor_120437 : public ScriptedAI
{
    ...
    */
    ...
    {
        EventMap().ScheduleEvent(SPELL_CORRUPTED_MATRIX, 10s);
    }
