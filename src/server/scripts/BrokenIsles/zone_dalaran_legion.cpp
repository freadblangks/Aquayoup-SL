...
public:
    ...
    void OnTempSummonNPC(uint32 insideNpc, Player* player)
    {
        ...
        TempSummon* personalCreature = player->SummonCreature(_insideNpc, _positon, TEMPSUMMON_TIMED_DESPAWN, 60000, 0);
    }
    ...
    void OnTempSummonNPCOnPOS(uint32 insideNpc, Player* player, Position npcPos)
    {
        ...
        _positon = npcPos;
        TempSummon* personalCreature = player->SummonCreature(_insideNpc, _positon, TEMPSUMMON_TIMED_DESPAWN, 3600000, 0);
    }
...
struct npc_archmage_khadgar_90417 : public ScriptedAI
{
    ...
    void QuestAccept(Player* player, Quest const* quest) override
    {
        ...
        {
            player->SummonGameObject(244874, Position(-851.257f, 4631.811f, 749.6389f, 1.389102f), QuaternionData(0.0f, 0.0f, 0.6400404f, 0.7683412f), 60000);
            Talk(2);
        }
   ...
   void QuestReward(Player* player, Quest const* quest, LootItemType /*type*/, uint32 /*opt*/) override
    ...
    void QuestAccept(Player* player, Quest const* quest) override
    {
        if (quest->GetQuestId() == 44004)
        {
            ...
            player->SummonGameObject(253930, Position(-805.065f, 4516.756f, 603.943f, 3.9149f), QuaternionData(), 0);
        }
    }
};
