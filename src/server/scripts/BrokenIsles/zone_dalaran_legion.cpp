...
public:
    ...
    void HandleLegionArrival(Player* player)
    {
        ...
        player->CastSpell(player,/* player->IsInAlliance() ?*/ SPELL_CREATE_CLASS_HALL_ALLIANCE, SPELL_CREATE_CLASS_HALL_HORDE);
...
// 113986 - Khadgar
class npc_dalaran_karazhan_khadgar : public PlayerScript
{
public:
    npc_dalaran_karazhan_khadgar() : PlayerScript("npc_dalaran_karazhan_khadgar") { }
...
// Zone 8392
class zone_legion_dalaran_underbelly : public PlayerScript
{
public:
    zone_legion_dalaran_underbelly() : PlayerScript("zone_legion_dalaran_underbelly") { }
...
// 108868 7.x.x
class npc_hunter_talua : public PlayerScript
{
public:
    npc_hunter_talua() : PlayerScript("npc_hunter_talua") { }
...
public:
    ...
    struct npc_great_eagleAI : public ScriptedAI
    {
        ...
        {
            me->GetMotionMaster()->MoveDistract(1000, true);
...
// 114561
class npc_khadgar_upgraded_servant : public PlayerScript
{
public:
    npc_khadgar_upgraded_servant() : PlayerScript("npc_khadgar_upgraded_servant") { }
...
// 86563
// 86563
class npc_archmage_khadgar_86563 : public PlayerScript
{
public:
    npc_archmage_khadgar_86563() : PlayerScript("npc_archmage_khadgar_86563") { }
    ...
    bool OnGossipSelect(Player* player, Creature* /*creature*/, uint32 /*sender*/, uint32 /*action*/)
    {
        if (player->hasQuest(QUEST_DOWN_TO_AZSUNA) || player->GetQuestStatus(QUEST_DOWN_TO_AZSUNA) == QUEST_STATUS_INCOMPLETE)
            player->CastSpell(player, /*player->IsInAlliance() ?*/ SPELL_TAXI_DALARAN_AZSUNA_ALLIANCE, SPELL_TAXI_DALARAN_AZSUNA_HORDE); // KillCredit & SendTaxi
...
public:
    ...
    bool OnGossipHello(Player* player, GameObject* go)
    {
        if (player->hasQuest(QUEST_ESTABLISHING_A_CONNECTION) &&
            !player->IsQuestObjectiveProgressBarComplete(QUEST_ESTABLISHING_A_CONNECTION, 0))
        {
        ...
        void OnLootStateChanged(uint32 /*state*/, Unit* unit) override
        {
            if (unit)
                if (unit->ToPlayer()->hasQuest(QUEST_ESTABLISHING_A_CONNECTION) &&
                    !unit->ToPlayer()->IsQuestObjectiveProgressBarComplete(QUEST_ESTABLISHING_A_CONNECTION, 0))
...
class go_a_mysterious_note : public PlayerScript
{
public:
    go_a_mysterious_note() : PlayerScript("go_a_mysterious_note") { }
...
//
class zone_legion_dalaran: public PlayerScript
{
public:
    zone_legion_dalaran() : PlayerScript("zone_legion_dalaran") { }
    uint32 _insideNpc;
    uint32 _killCredit;
    Creature* creature;
    Position _positon;
...
public:
    ...
    bool OnGossipHello(Player* player, Creature* creature)
    {
        if (player->GetQuestStatus(41121) == QUEST_STATUS_INCOMPLETE && !_diffsPicked && !_noTimePicked)
            AddGossipItemFor(player, GossipOptionIcon::None, GOSSIP_HELLO_ALTURAS1, GOSSIP_SENDER_MAIN, GOSSIP_ACTION_INFO_DEF + 1);

        if (player->GetQuestStatus(41121) == QUEST_STATUS_INCOMPLETE && !_diffsPicked)
            AddGossipItemFor(player, GossipOptionIcon::None, GOSSIP_HELLO_ALTURAS2, GOSSIP_SENDER_MAIN, GOSSIP_ACTION_INFO_DEF + 2);

        if (player->GetQuestStatus(41121) == QUEST_STATUS_INCOMPLETE && _diffsPicked)
            AddGossipItemFor(player, GossipOptionIcon::None, GOSSIP_HELLO_ALTURAS3, GOSSIP_SENDER_MAIN, GOSSIP_ACTION_INFO_DEF + 3);

        if (player->GetQuestStatus(41121) == QUEST_STATUS_INCOMPLETE && _diffsPicked)
            AddGossipItemFor(player, GossipOptionIcon::None, GOSSIP_HELLO_ALTURAS4, GOSSIP_SENDER_MAIN, GOSSIP_ACTION_INFO_DEF + 4);
...
class go_violethold_entrance_portal : public PlayerScript
{
public:
    go_violethold_entrance_portal() : PlayerScript("go_violethold_entrance_portal")
    {
        checkTimer = 1000;
        giveKillCredit = 1000;
    }
            ...
            while (uint32 eventId = _events.ExecuteEvent())
            {
                ...
                case EVENT_TELEPORT_BROKENSHORE:
                    ...
                    if (Player* player = ObjectAccessor::GetPlayer(*me, _playerGUID)) {
                        ...
                        player->TeleportTo(1500, -2379.679f, 174.2f, 3.5625f, 3.733872f);
                    }
                ...
                case EVENT_TELEPORT_SURAMAR:
                    if (Player* player = ObjectAccessor::GetPlayer(*me, _playerGUID)) {
                        ...
                        player->TeleportTo(1498, 1263.69f, 5236.659f, 93.531f, 2.73693f);
                    }
                    break;
                default:
                    break;
...
class spell_redoubt_teleport_to_dh_ch : public SpellScript
{
    ...
    void HandleOnCast()
    {
        if (Player* player = GetCaster()->ToPlayer()) {
            ...
            player->GetAchievementMgr()->UpdateCriteria(CriteriaType::BeSpellTarget, 215782, 0, 0, player);
        }
    }
