...
struct npc_malfurion_stormrage_106482 : public ScriptedAI
{
    ...
    bool GossipHello(Player* player) override
    {
        ...
        AddGossipItemFor(player, GossipOptionIcon::None, tempstr, GOSSIP_SENDER_MAIN, GOSSIP_ACTION_INFO_DEF + 2);
    ...
    bool GossipSelect(Player* player, uint32 /*menuId*/, uint32 gossipListId) override
    {
        ...
        if (instance)
            player->TeleportTo(1520, 11196.4f, -12614.2f, 549.707f, 20.4509f);
...
struct npc_bolan_the_marked_111860 : public ScriptedAI
{
    ...
    {
        EventMap().ScheduleEvent(SPELL_CRUSH_ARMOR, 3s, 5s);
    }

    void DamageTaken(Unit* /*attacker*/, uint32& damage) override
    {
        if(me->HealthBelowPctDamaged(40, damage))
            EventMap().ScheduleEvent(SPELL_INTIMIDATING_ROAR, 2s);
    }
...
struct npc_corrupted_feeler_113088 : public ScriptedAI
{
    ...
    {
        EventMap().ScheduleEvent(SPELL_CORRUPT, 5s);
    }
...
struct npc_flail_of_ilgynoth_113920 : public ScriptedAI
{
    ...
    void EnterCombat(Unit* /*attacker*/)
    {
        EventMap().ScheduleEvent(SPELL_SPEW_ICHOR, 5s);
        EventMap().ScheduleEvent(SPELL_WILD_FLAIL, 5s, 8s);
    }
