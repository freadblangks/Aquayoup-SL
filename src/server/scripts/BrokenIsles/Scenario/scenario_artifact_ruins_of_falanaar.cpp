...
struct scenario_artifact_ruins_of_falanaar : public InstanceScript
{
    ...
    void SetData(uint32 type, uint32 data) override
    {
        ...
        {
            MovieEntry(DRUID_FERAL_ACQUISTION_ARTIFACT_3_SCENE);
            NextStep();
        }
        ...
        {
            NextStep();
            MovieEntry(DRUID_FERAL_ACQUISTION_ARTIFACT_LOOTED_SCENE);
        }
...
class go_ruined_switch : public PlayerScript
{
public:
    go_ruined_switch() : PlayerScript("go_ruined_switch") { isused = false; }
...
///250824
class go_platform : public PlayerScript
{
public:
    go_platform() : PlayerScript("go_platform") { isused = false; }
    ...
    bool OnGossipHello(Player* player, GameObject* go)
    {
        if (InstanceScript * instance = go->GetInstanceScript())
        {
            ...
            {
                isused = true;
                MovieEntry(DRUID_FERAL_ACQUISTION_ARTIFACT_SECOND_DOOR_SCENE);
                //jumpto or tele to
...
struct npc_verstok_darkbough_108003 : public BossAI
{
    ...
    void DoAction(int32 action) override
    {
        if (action == ACTION_INTRO)
        {
            ...
            me->GetMotionMaster()->MovePoint(0, true);
            me->RemoveUnitFlag(UnitFlags(UNIT_FLAG_IMMUNE_TO_PC));
            me->AddUnitFlag(UnitFlags(UNIT_FLAG_UNK_15));
        }
...
///250386
class go_fangs_of_ashamane : public PlayerScript
{
public:
    go_fangs_of_ashamane() : PlayerScript("go_fangs_of_ashamane") { isloot = false; }
...
struct npc_ebonfang_108115 : public ScriptedAI
{
    ...
    void OnSpellClick(Unit* clicker, bool& /*result*/)
    {
        ...
        if (Player* player = clicker->ToPlayer())
            player->TeleportTo(1000, 1220, 3975.49f, 7389.64f, 23.91f, 1.575509f);
    }
};
