...
struct scenario_artifact_brokenshore : public InstanceScript
{
    ...
    void SetData(uint32 type, uint32 data) override
    {
        ...
        {
            NextStep();
            SummonAldrachiWarblades(nullptr); //summon Aldrachi Warblades GO + NPC
        }
        else if (type == DATA_STAGE_7 && data == DONE)
        {
            isComplete = true;
            NextStep();
            MovieEntry(DH_VENGEANCE_ARTIFACT_ACQUISTION_LOOTED_SCENE);
            SummonFelBat();
        }
    }
    ...
    void SummonFelsoulPortals(Player* go)
    {
        GameObject* felsoulPortal1 = go->SummonGameObject(GO_FELSOUL_PORTAL_1, Position(-2648.52f, 19.5848f, 48.6442f, 0.85781f), QuaternionData(-0.0f, -0.0f, -0.415875f, -0.909422f), 300);
        GameObject* felsoulPortal2 = go->SummonGameObject(GO_FELSOUL_PORTAL_2, Position(-2618.59f, 78.8602f, 38.1285f, 6.15595f), QuaternionData(-0.0f, -0.0f, -0.0635752f, 0.997977f), 300);
    ...
    void SummonAldrachiWarblades(Player* go)
    {
        TempSummon* aldrachiWb = instance->SummonCreature(NPC_ALDRACHI_WARBLADES, Position(-2746.991f, -328.47f, 38.4056f, 2.2876f));
        GameObject* aldrachiWbLoot = go->SummonGameObject(GO_ALDRACHI_WARBLADES, Position(-2747.71f, -328.3544f, 38.4344f, 2.28768f), QuaternionData(), 300);
    }
    ...
    void SummonCollapsedRocks(Player* go) {
        GameObject* aldrachiWbLoot = go->SummonGameObject(GO_STONE_WALL, Position(-2740.67f, -149.12f, 48.4044f, 1.5051f), QuaternionData(), 300);
    }
...
class go_brokenshore_felsoul_portal : public PlayerScript
{
public:
    go_brokenshore_felsoul_portal() : PlayerScript("go_brokenshore_felsoul_portal") {}
...
class go_temporary_allari_cage : public PlayerScript
{
public:
    go_temporary_allari_cage() : PlayerScript("go_temporary_allari_cage") { }
        ...
        void UpdateAI(uint32 diff) override
        {
            ...
            while (uint32 eventId = _events.ExecuteEvent())
            {
                ...
                case EVENT_CALL_OF_THE_WARBLADES:
                    ...
                    EventMap().ScheduleEvent(EVENT_CAST_CALL_WARBLADES, 1300);
                    break;
                case EVENT_CAST_CALL_WARBLADES:
                    ...
                    EventMap().ScheduleEvent(EVENT_RESTORE_STATE, Seconds(10));
                    break;
...
class go_aldrachi_warblades_248785 : public PlayerScript
{
public:
    go_aldrachi_warblades_248785() : PlayerScript("go_aldrachi_warblades_248785") {
        isLooted = false;
        prevState = 0;
    ...
    void OnLootStateChanged(GameObject* go, uint32 state, Unit* unit)
    {
        ...
        if (state == GO_JUST_DEACTIVATED && prevState == GO_ACTIVATED && playerGuid != ObjectGuid::Empty && !isLooted)
        {
            ...
            if (Player* player = ObjectAccessor::GetPlayer(*go, playerGuid)) {
                if (!player->IsQuestObjectiveProgressBarComplete(42430, 0))
...
class go_cavern_stones_7796 : public PlayerScript
{
public:
    go_cavern_stones_7796() : PlayerScript("go_cavern_stones_7796") { isUsed = false; }
            ...
            while (uint32 eventId = _events.ExecuteEvent())
            {
                ...
                case EVENT_TELEPORT:
                    ...
                    if (Player* player = ObjectAccessor::GetPlayer(*me, _playerGUID)) {
                        ...
                        player->TeleportTo(1220, -839.796f, 4259.4711f, 746.2744f, 1.207325f);
                    }
                    me->DespawnOrUnsummon();
                    break;
                default:
