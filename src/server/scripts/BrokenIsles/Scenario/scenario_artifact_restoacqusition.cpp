    ...
    void SetData(uint32 type, uint32 data) override
    {
        ...
        if (type == DATA_STAGE_1 && data == DONE)
        {
            ...
            if (Creature* omnuron = instance->GetCreature(_omnuronGUID))
            {
                ...
                omnuron->GetMotionMaster()->MoveTakeoff(15.0f, 3000);
            }
        ...
        else if (Player* go = go)
            if (type == DATA_STAGE_5 && data == DONE)
        {
            NextStep();
            go->SummonGameObject(248499, Position(5474.0f, -3501.37f, 1557.3f), QuaternionData(), WEEK);
        }
            if (type == DATA_STAGE_6 && data == DONE)
        {
            if (Creature* hamuul = instance->GetCreature(_hamuulGUID))
                    hamuul->Say(108260);
            ...
            if (Creature* lyessa = instance->GetCreature(_lyessaGUID))
                lyessa->Say(108261);
            NextStep();
            MovieEntry(DRUID_RESTO_ACQUISTION_ARTIFACT_LOOTED_SCENE);
...
///248499
class go_g_hanir_1 : public PlayerScript
{
public:
    go_g_hanir_1() : PlayerScript("go_g_hanir_1") { isloot = false; }
