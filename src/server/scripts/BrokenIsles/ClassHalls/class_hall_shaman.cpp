    ...
    void OnSpellClick(Unit* clicker, bool& /*result*/)
    {
        if (Player* player = clicker->ToPlayer())
        {
            ...
            if (player->GetQuestStatus(39746) == QUEST_STATUS_INCOMPLETE)
            {
                ...
                player->TeleportTo(1325, 1572, 827.552f, 1037.688f, 48.341f, 5.8136f);
            }
        }
    }
...
struct npc_sturmschnabel_98383 : public ScriptedAI
{
   ...
   {
                me->SetAIAnimKitId(4061);
                player->TeleportTo(1325, 646, 1029.35f, 668.829f, 156.672f, 4.37562f);
            }
...
struct npc_sturmschnabel_983830 : public ScriptedAI
{
   ...
   {
                me->SetAIAnimKitId(4061);
                player->TeleportTo(1325, 1469, 813.921f, 1039.71f, 48.4425f, 5.8136f);
            }
...
public:
  ...
  void JustDied(Unit* who/*killer*/) override
     ...
     //if (instance)
     //player->DoPlayScenePackageIdOnPlayers(1491);
                }
   }
    ...
    void OnSpellClick(Unit* clicker, bool& /*result*/)
    {
        if (Player* player = clicker->ToPlayer())
        {
            if (player->GetQuestStatus(43644) == QUEST_STATUS_INCOMPLETE)
            {
                ...
                player->TeleportTo(1325, 0, -6796.7f, 4565.75f, -605.263f, 3.01229f);
...
class go_sharas_dal_249342 : public PlayerScript
{
public:
    go_sharas_dal_249342() : PlayerScript("go_sharas_dal_249342")
    {
        isloot = false;
    }

    void OnLootStateChanged(GameObject* go, uint32 state, Unit* unit, Player* player)
    {
        if (state == GO_ACTIVATED && unit && !isloot)
        {
            isloot = true;
            player->GetScheduler().Schedule(5s, [this, go, unit](TaskContext /*context*/)
            {
                if (InstanceScript* instance = go->GetInstanceScript())
                {
                    instance->SendBossKillCredit(40341);
...
class go_highborne_203213 : public PlayerScript
{
public:
    go_highborne_203213() : PlayerScript("go_highborne_203213") { }
...
class go_highborne_203218 : public PlayerScript
{
public:
    go_highborne_203218() : PlayerScript("go_highborne_203218") { }
...
class go_highborne_203212 : public PlayerScript
{
public:
    go_highborne_203212() : PlayerScript("go_highborne_203212") { }
         ...
         void MoveInLineOfSight(Unit* who) override
        {
            if (Player* player = who->ToPlayer())
            {
                if (player->GetQuestStatus(40341) == QUEST_STATUS_INCOMPLETE)
                {
                    ...
                    {
                        player->TeleportTo(1325, 1600, -629.42f, 808.68f, 247.67f, 6.25976f);
                    }
                }
            }
        }
...
struct npc_bubble_1121423 : public ScriptedAI
{
    ...
    void OnSpellClick(Unit* clicker, bool& /*result*/)
    {
        if (Player* player = clicker->ToPlayer())
        {
            ...
            {
                me->SetAIAnimKitId(4061);
                player->TeleportTo(1325, 1600, -184.989f, 802.404f, 796.645f, 6.25583f);
                player->CastSpell(player, SPELL_CAST_GOB, true);
            }
        }
    }
    ...
    void OnSpellClick(Unit* clicker, bool& /*result*/)
    {
        if (Player* player = clicker->ToPlayer())
        {
            if (player->GetQuestStatus(40341) == QUEST_STATUS_INCOMPLETE)
            {
                ...
                player->TeleportTo(1325, 1469, 824.465f, 1036.18f, 48.26f, 1.46014f);
    ...
    void OnSpellClick(Unit* clicker, bool& /*result*/)
    {
        if (Player* player = clicker->ToPlayer())
        {
            ...
            if (player->GetQuestStatus(43334) == QUEST_STATUS_INCOMPLETE)
            {
                ...
                player->TeleportTo(1325, 870, 3265.64f, 656.921f, 538.276f, 5.49793f);
            }
        }
    }
