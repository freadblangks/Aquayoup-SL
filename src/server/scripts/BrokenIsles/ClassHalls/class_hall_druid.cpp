    ...
    struct npc_class_hall_druid_gatewardenAI : public ScriptedAI
    {
        ...
        void MoveInLineOfSight(Unit* who) override
        {
            ...
            if (me->isInBackInMap(who, 12.0f))   // In my line of sight, "outdoors", and behind me
            {
                ...
                {
                    if (!who->ToPlayer()->IsQuestObjectiveProgressBarComplete(QUEST_TO_THE_DREAMGROVE, 0))
                    {
                ...
                {
                    if (!who->ToPlayer()->IsQuestObjectiveProgressBarComplete(QUEST_TO_THE_HILLS, 0))
                    {
                ...
                {
                    if (!who->ToPlayer()->IsQuestObjectiveProgressBarComplete(QUEST_THE_SCYTHE_OF_ELUNE, 0))
                    {
...
struct npc_zentabra_103135 : public ScriptedAI
{
    ...
    void MoveInLineOfSight(Unit* who) override
    {
        ...
        if (!HasPlayer(player->GetGUID()) && player->hasQuest(QUEST_CALL_OF_THE_WILDS) && !player->IsQuestObjectiveProgressBarComplete(QUEST_CALL_OF_THE_WILDS,0) )
        {
            m_playerGUID = player->GetGUID();
            AddPlayer();
...
// 103875 Malfurion Stormrage
    struct npc_malfurion_stormrage_103875 : public ScriptedAI
    {
        ...
        void MoveInLineOfSight(Unit* who) override
        {
            ...
            {
                if (!player->IsQuestObjectiveProgressBarComplete(QUEST_THE_DREAMWAY, 0))
                {
                ...
                else if (!player->IsQuestObjectiveProgressBarComplete(QUEST_THE_DREAMWAY, 0))
                {
...
public:
    ...
    void OnMapChanged(Player *player) override
    {
        if (player->hasQuest(QUEST_TO_THE_DREAMGROVE)&& player->GetMapId()==1540)
        {
            ...
            if (!player->IsQuestObjectiveProgressBarComplete(QUEST_TO_THE_DREAMGROVE, 0))
            {
                player->CastSpell(player, 204836, true);
                PhasingHandler::AddPhase(player, 6122, true);
            }
            else if (!player->IsQuestObjectiveProgressBarComplete(QUEST_TO_THE_DREAMGROVE, 0))
            {
                PhasingHandler::AddPhase(player, 5953, true);
                PhasingHandler::AddPhase(player, 6122, true);
            }

            if (player->IsQuestObjectiveProgressBarComplete(QUEST_TO_THE_DREAMGROVE, 0)|| player->GetQuestStatus(QUEST_TO_THE_DREAMGROVE)== QUEST_STATUS_COMPLETE || player->GetQuestStatus(QUEST_TO_THE_DREAMGROVE) == QUEST_STATUS_REWARDED)
            {
...
private:
    ...
    struct npc_rensar_greathoof_101195 : public ScriptedAI
    {
        ...
        void MoveInLineOfSight(Unit* who) override
        {
            ...
            if (!player)
                return;
            if (!isTalking && player->GetQuestStatus(QUEST_TO_THE_DREAMGROVE) == QUEST_STATUS_INCOMPLETE && !player->IsQuestObjectiveProgressBarComplete(QUEST_TO_THE_DREAMGROVE, 0))
            {
                isTalking = true;
    ...
    struct npc_lea_stonepaw_104535 : public ScriptedAI
    {
        ...
        void MoveInLineOfSight(Unit* who) override
        {
            ...
            if (!player)
                return;
            if (player->hasQuest(QUEST_TO_THE_HILLS) && !player->IsQuestObjectiveProgressBarComplete(QUEST_TO_THE_HILLS, 0))
            {
    ...
    public:
        ...
        void OnSuccessfulSpellCast(Player* player, Spell* spell)
        {
            if (spell->GetSpellInfo()->Id == 206807)
            {
                ...
                player->TeleportTo(2000, 1536, -12408.0f, -12955.2f, 320.26f, 4.427202f);
            }
        }
    };
    ...
    ///248098
    class go_g_hanir : public PlayerScript
    {
    public:
        go_g_hanir() : PlayerScript("go_g_hanir") { }
        ...
        void OnLootStateChanged(GameObject* go, uint32 state, Unit* unit)
        {
            if (state == GO_ACTIVATED && unit)
            {
                ...
                if (Player* player = unit->ToPlayer())
                {
                    ...
                    {
                        player->TeleportTo(1220, 4299.72f, 7422.46f, -17.13f, 2.535614f);
                    });
                }
    ...
    struct npc_eredar_soul_lasher_107535 : public ScriptedAI
    {
        ...
        void JustDied(Unit* /*killer*/) override
        {
            if (Creature* druid = me->FindNearestCreature(107389, 15.0f, true))
            {
                ...
                druid->GetMotionMaster()->MoveTakeoff(10.0f, 2000);
            }
        }
    };
        ...
        void MoveInLineOfSight(Unit* who) override
        {
            if (Player* player = who->ToPlayer())
                if (player->GetDistance(me) < 5.0f)
                    if (!player->IsQuestObjectiveProgressBarComplete(QUEST_THE_SHRINE_IN_PERIL, 0))
                    {
                        player->KilledMonsterCredit(me->GetEntry());
        ...
        void OnSpellClick(Unit* clicker, bool& /*result*/)
        {
            if (Player* player = clicker->ToPlayer())
                if(player->hasQuest(42430))
                    player->TeleportTo(1612, 1940.97f, 5558.38f, 60.66f, 4.746947f);
        }
    };
    ...
    struct npc_valorn_stillbough_101656 : public ScriptedAI
    {
        ...
        void DoAction(int32 param) override
        {
            ...
            case ACTION_QUEST_ITS_RIGHTFUL_PLACE:
                if (Player* player = ObjectAccessor::GetPlayer(*me, m_playerGUID))
                {
                    if (!issay && !player->IsQuestObjectiveProgressBarComplete(QUEST_ITS_RIGHTFUL_PLACE, 0))
                    {
                        issay = true;
                        ...
                        ariden->GetMotionMaster()->MoveTakeoff(50.0f, 5000);
                        me->GetScheduler().Schedule(Milliseconds(4000), [this](TaskContext context)
                        {
                            issay = false;
                        });
        ...
        void MoveInLineOfSight(Unit* who) override
        {
            ...
            if (player->hasQuest(QUEST_THE_SCYTHE_OF_ELUNE) && player->GetQuestStatus(QUEST_THE_SCYTHE_OF_ELUNE) == QUEST_STATUS_INCOMPLETE && !player->IsQuestObjectiveProgressBarComplete(QUEST_THE_SCYTHE_OF_ELUNE, 0) )
            {
                me->Say(104053);
            ...
            if (player->hasQuest(QUEST_ITS_RIGHTFUL_PLACE) && player->GetQuestStatus(QUEST_ITS_RIGHTFUL_PLACE) == QUEST_STATUS_INCOMPLETE && !player->IsQuestObjectiveProgressBarComplete(QUEST_ITS_RIGHTFUL_PLACE, 0))
                DoAction(ACTION_QUEST_ITS_RIGHTFUL_PLACE);
        }
    private:
        bool issay;
    ...
    struct npc_revil_kost_100323 : public ScriptedAI
    {
        ...
        void QuestAccept(Player* player, Quest const* quest) override
        {
            if (quest->GetQuestId() == QUEST_FOLLOWING_THE_CURSE)
            {
                ...
                revil->GetMotionMaster()->MovePoint(1, revil->GetPositionWithOffset(25.0f), true);
                //WP START
                revil->AI()->Talk(0);
        ...
        void MoveInLineOfSight(Unit* who) override
        {
            ...
            if (!player)
                return;
            m_playerGUID = player->GetGUID();
            if (/*!IsLock &&*/ player->hasQuest(QUEST_A_FOE_OF_THE_DARK) && player->GetQuestStatus(QUEST_A_FOE_OF_THE_DARK) == QUEST_STATUS_INCOMPLETE && !player->IsQuestObjectiveProgressBarComplete(QUEST_A_FOE_OF_THE_DARK, 0))
            {
                //IsLock = true;
    ...
    struct npc_revil_kost_100812 : public ScriptedAI
    {
        ...
        void QuestAccept(Player* player, Quest const* quest) override
        {
            if (quest->GetQuestId() == QUEST_THE_DARK_RIDERS)
            {
                ...
                player->TeleportTo(1533, -11018.37f, -2002.739f, 24.99422f, 1.369567f);
