...
struct npc_meryl_felstorm_102700 : public ScriptedAI
{
    ...
    bool GossipHello(Player* player) override
    {
        ...
        if (player->hasQuest(QUEST_FINDING_BONCHILL) && (player->IsQuestObjectiveProgressBarComplete(QUEST_FINDING_BONCHILL, 0) ) && !player->IsQuestObjectiveProgressBarComplete(QUEST_FINDING_BONCHILL, 0))
        {

            //menu (19321, 0, 0, 'Tell Merrill about the information you found.', 0),
            ClearGossipMenuFor(player);
            AddGossipItemFor(player, GossipOptionIcon::None, "Tell Merrill about the information you found.", GOSSIP_SENDER_MAIN, GOSSIP_ACTION_INFO_DEF);
    ...
    void MoveInLineOfSight(Unit* who) override
    {
        ...
        if (!HasPlayer(player->GetGUID()) && (player->hasQuest(QUEST_THE_DREADLORDS_PRIZE)) &&(!player->IsQuestObjectiveProgressBarComplete(QUEST_THE_DREADLORDS_PRIZE,0) ))
        {
            m_playerGUID = player->GetGUID();
            AddPlayer();
        ...
        ///conversation 1281 ?????!???? ?????
        if (!HasPlayer(player->GetGUID()) && (player->hasQuest(QUEST_THE_DREADLORDS_PRIZE)) && (player->IsQuestObjectiveProgressBarComplete(QUEST_THE_DREADLORDS_PRIZE, 0)) && (!player->IsQuestObjectiveProgressBarComplete(QUEST_THE_DREADLORDS_PRIZE, 0)))
        {
    ...
    void QuestAccept(Player* player, Quest const* quest) override
    {
        if (quest->GetQuestId() == QUEST_THE_DREADLORDS_PRIZE)
        {
           ...
           player->TeleportTo(14000, 1494, 1276.16f, -263.27f, 44.36f, 6.275331f);
...
class go_finding_bonchill_note : public PlayerScript
{
public:
    go_finding_bonchill_note() : PlayerScript("go_finding_bonchill_note") { }

    bool OnGossipHello(Player* player, GameObject* go)
    {
        ///TC_LOG_ERROR("server.worldserver", "go_finding_bonchill_note %u", go->GetEntry());
        if (!player->IsQuestObjectiveProgressBarComplete(QUEST_FINDING_BONCHILL, 0))
        {
            switch (go->GetEntry())
            {
            case 250549:
