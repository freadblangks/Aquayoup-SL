...
struct npc_latosius : public ScriptedAI
{
    ...
    void EnterCombat(Unit* /*attacker*/)
    {
        if (IsLatosius())
        {
            ...
            EventMap().ScheduleEvent(SPELL_DARK_BLAST, 10s);
            EventMap().ScheduleEvent(SPELL_SHADOW_BOLT, 5s);
        }
        else
        {
            EventMap().ScheduleEvent(SPELL_SHADOW_BOLT_VOLLEY,          5s);
            EventMap().ScheduleEvent(SPELL_CLOUD_OF_HYPNOSIS_SUMMON,    10s);
            EventMap().ScheduleEvent(SPELL_DREADLORDS_GUILE,            20s);

            if (IsHeroic())
                EventMap().ScheduleEvent(SPELL_STINGING_SWARM, 1s);
        }
    }
    ...
    void DoAction(int32 action) override
    {
        ...
        if (IsLatosius() && action == SPECIAL)
        {
            ...
            Schedule(9s, [this](TaskContext /*context*/)
            {
                ...
                if (Creature* kurtalos = me->CreateCreature(NPC_LORD_KURTALOS_RAVENCREST, nullptr, 0))
                {
    ...
    void UpdateAI(uint32 diff) override
    {
        ...
        switch (EventMap().ExecuteEvent())
        {
            ...
            case SPELL_DREADLORDS_GUILE:
                ...
                EventMap().Repeat(30s);
                EventMap().DelayEvents(25s); // 5s spell cast + 17s event + 2s security
                break;
    ...
    void OnSuccessfulSpellCast(SpellInfo const* spell) override
    {
        if (spell->Id == SPELL_DREADLORDS_GUILE)
        {
            ...
            me->GetScheduler().Schedule(1s, [this](TaskContext /*context*/)
            {
                ...
                me->SetFacingTo(me->GetAngle2(centerPosition));
                me->SetOrientation(me->GetAngle2(centerPosition));
...
struct npc_dantalionax_stinging_swarm : public ScriptedAI
{
    ...
    void Reset() override
    {
        Player* target = me->SelectNearestPlayer(-1);
