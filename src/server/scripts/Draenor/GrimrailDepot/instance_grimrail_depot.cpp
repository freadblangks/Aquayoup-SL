    ...
    void OnUnitDeath(Unit* p_Unit) override
    {
        ...
        case GrimrailDepotBosses::BossSkylordTovra:
        {
            if (l_Creature->GetMap()->IsHeroic())
                DoCompleteAchievement2(GrimrailDepotAchivement::AchievementGrimrailDepotHeroic);
            else
                DoCompleteAchievement2(GrimrailDepotAchivement::AchievementGrimrailDepotNormal);
            break;
        }
        default:
            break;
