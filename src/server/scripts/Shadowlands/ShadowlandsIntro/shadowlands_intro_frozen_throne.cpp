...
struct npc_chilling_summons_commander : public ScriptedAI
{
    ...
    {
        AddGossipItemFor(player, GossipOptionIcon::Unk41, "Tell me what happened.", GOSSIP_SENDER_MAIN, GOSSIP_ACTION_INFO_DEF + 0);
    ...
    bool GossipSelect(Player* player, uint32 /*menuId*/, uint32 /*gossipListId*/) override
    {
        if (player->GetQuestStatus(QUEST_CHILLING_SUMMONS_A) == QUEST_STATUS_INCOMPLETE || player->GetQuestStatus(QUEST_CHILLING_SUMMONS_H) == QUEST_STATUS_INCOMPLETE)
        {
            ...
            player->CompleteQuest(QUEST_CHILLING_SUMMONS_A);
            player->CompleteQuest(QUEST_CHILLING_SUMMONS_H);
