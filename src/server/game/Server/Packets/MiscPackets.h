...
namespace WorldPackets
{
    namespace Misc
    {
        class NewDataBuild final : public ServerPacket
        {
        public:
            NewDataBuild(std::array<uint8, 16> b, std::array<uint8, 16> c) : ServerPacket(SMSG_NEW_DATA_BUILD, 32), NewBuildKey(b), NewCDNKey(c) { }

            WorldPacket const* Write() override;

            std::array<uint8, 16> NewBuildKey = { };
            std::array<uint8, 16> NewCDNKey = { };
        };
